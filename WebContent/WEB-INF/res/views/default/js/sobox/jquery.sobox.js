...
(function($) {
	...
	$so.base = {
		...
		// set position,=ie6+
		setPos : function(o) {
			var $o = $(o), t = Math.floor($o.height() / 2), l = Math.floor($o.width() / 2);
		...
		show : function(obj, fn) {// 显示弹出框和mask,fn返回事件
			...
			// $('.so-openmask').bind('click',function ()
			// {$so.base.hide(obj);});
	...
	$.soPopbox = $so.pop = function(o) {
		var o = $.extend({
			...
			btn : []
		// {className:,name:'确定',link:,returnFalse:true,closePop:
		// true,callback:}
		}, o || {});
		var $popWrap = $('<div class="so-popbox" style="width:' + o.width + 'px;visibility:hidden;"></div>');
		var $mask;// 在base.show事件中设置
		if (o.className) {
			$popWrap.addClass(o.className);
		}
		if (o.height) {
			$popWrap.css('height', o.height + 'px');
		}
		...
		/*修改弹窗标题*/
		$(".so-popbox .s-sopop-title").html(o.title);
		...
		if (o.btn) {// 添加按钮
			...
			$.each(o.btn, function() {
				...
				if (param.className !== null) {
					thisBtn.addClass(param.className);
				}
				thisBtn.bind('click', function() {
					if (param.callback) {
						param.callback(removePop);
					}
					if (param.closePop) {
						removePop();
					}
